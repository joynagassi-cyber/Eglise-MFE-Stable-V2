...
void main() {
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.enterText(find.text('Email'), 'jeunesse-test-${DateTime.now().millisecondsSinceEpoch}@lumina.app');
      await tester.enterText(find.text('Mot de passe'), 'Test123!');
      final nameField = find.byType(TextField).at(2);
      if (nameField.evaluate().isNotEmpty) {
        await tester.enterText(nameField, 'Leader Jeunesse Test');
      }
      await tester.tap(find.text('S\'inscrire'));
      ...
      if (find.text('CODE DE RÔLE').evaluate().isNotEmpty) {
        await tester.enterText(find.text('CODE DE RÔLE'), 'PRESIDENT-JEUNESSE-XXXX-2026');
        await tester.pumpAndSettle();
