...
class BibleRepositoryImpl implements IBibleRepository {
  ...
  // ── METADATA ────────────────────────────────────────────
  // Les constantes sont dans bible_constants.dart

  // ── LIVRES ──────────────────────────────────────────────

  @override
  List<BibleBook> getAllBooks() {
    return canonBibleBooks.map((b) {
      return BibleBook(
        identifier: b['id']!,
        name: b['name']!,
        chapterCount: bibleChapterCounts[b['id']] ?? 1,
        testament: b['t'] == 'old'
            ? BibleTestament.old
  ...
  BibleBook? getBookByIdentifier(String identifier) {
    try {
      final b = canonBibleBooks.firstWhere((b) => b['id'] == identifier.toUpperCase());
      return BibleBook(
        identifier: b['id']!,
        name: b['name']!,
        chapterCount: bibleChapterCounts[b['id']] ?? 1,
  ...
  String _getBookName(String bookId) {
    try {
      return canonBibleBooks.firstWhere((b) => b['id'] == bookId)['name'] ?? bookId;
    } catch (_) {
      return bookId;
    }
  }
